...
export function ClientReportsPage() {
  ...
  return (
    <div className="p-6 md:p-8 max-w-7xl mx-auto space-y-8 animate-in fade-in duration-500">
      {/* Page Header */}
      <PageHeader
        title={isAr ? "تقارير العملاء المستندية" : "Client Reports Center"}
        description={
          isAr
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <div className="glass-card p-5 rounded-2xl flex items-center justify-between transition-all hover:scale-[1.01]">
          <div>
            <span className="text-xs font-semibold text-default-500 block">
        ...
        <div className="glass-card p-5 rounded-2xl flex items-center justify-between transition-all hover:scale-[1.01]">
          <div>
            ...
            <span className="text-2xl font-bold text-foreground mt-1 block">{stats.thisMonthCount}</span>
          </div>
        ...
        <div className="glass-card p-5 rounded-2xl flex items-center justify-between transition-all hover:scale-[1.01]">
          <div>
            ...
            <span className="text-2xl font-bold text-warning mt-1 block">{stats.pendingActionItems}</span>
          </div>
          <div className="p-3 bg-amber-500/10 text-warning rounded-xl border border-amber-500/20">
        ...
        <div className="glass-card p-5 rounded-2xl flex items-center justify-between transition-all hover:scale-[1.01]">
          <div>
            ...
            <span className="text-2xl font-bold text-foreground mt-1 block flex items-center gap-1">
              {stats.avgRating} <Star className="w-4 h-4 text-amber-400 fill-amber-400" />
            </span>
          </div>
          <div className="p-3 bg-purple-500/10 text-secondary rounded-xl border border-purple-500/20">
      ...
      <div className="glass-card p-4 rounded-2xl flex flex-col md:flex-row gap-4 justify-between items-center">
        ...
        <div className="flex flex-wrap items-center gap-3 w-full md:w-auto">
          ...
          <select
            ...
            className="bg-content1 text-foreground border border-default-200 dark:border-default-100/60 text-sm rounded-xl px-3 py-2 focus:outline-none focus:border-primary shadow-xs"
          >
      ...
      {isLoading ? (
        ...
        <div className="glass-card text-center py-16 rounded-2xl p-8">
          <FileText className="w-12 h-12 text-default-300 dark:text-default-600 mx-auto mb-3" />
          <h3 className="text-lg font-bold text-foreground">
            {isAr ? "لا توجد تقارير عملاء مطابقة" : "No client reports found"}
          </h3>
          <p className="text-sm text-default-400 max-w-md mx-auto mt-1">
            {isAr
          ...
          {filteredReports.map((report) => {
            ...
            return (
              <div
                ...
                className="glass-card group relative hover:border-primary/50 rounded-2xl p-6 transition-all cursor-pointer hover:shadow-2xl hover:-translate-y-1 flex flex-col justify-between"
              >
                <div>
                  {/* Top Bar */}
                  ...
                  <div className="space-y-2 text-xs text-default-500 mb-4 bg-default-100/50 dark:bg-default-50/40 p-3 rounded-xl border border-default-200/60 dark:border-default-100/40">
                    <div className="flex items-center gap-2">
                      <User className="w-4 h-4 text-primary shrink-0" />
                      <span className="font-semibold text-default-600">{isAr ? "العميل:" : "Client:"}</span>
                      <span className="text-foreground font-medium truncate">{report.contactName}</span>
                    </div>

                    <div className="flex items-center gap-2">
                      <FileText className="w-4 h-4 text-secondary shrink-0" />
                      <span className="font-semibold text-default-600">{isAr ? "الموظف:" : "Author:"}</span>
                      <span className="text-foreground font-medium truncate">{report.authorName}</span>
                    </div>
                  </div>

                  {/* Snippet / Description */}
                  <p className="text-xs text-default-500 dark:text-default-400 line-clamp-3 leading-relaxed mb-4 font-normal">
                    {report.description || report.content.replace(/[#*>]/g, "") || (isAr ? "لا يوجد وصف أو ملخص نصي" : "No description")}
                  </p>
                </div>

                {/* Card Footer */}
                <div className="pt-4 border-t border-default-200/60 dark:border-default-100/40 flex items-center justify-between">
                  ...
                  <div className="flex items-center gap-3">
                    {pendingTasksCount > 0 && (
                      <span className="text-xs bg-amber-500/10 text-amber-500 border border-amber-500/20 px-2 py-0.5 rounded-md font-semibold">
                        {pendingTasksCount} {isAr ? "مهام" : "tasks"}
                      </span>
                    )}
                    <div className="flex items-center gap-0.5">
                      <Star className="w-3.5 h-3.5 text-amber-400 fill-amber-400" />
                      <span className="text-xs font-bold text-foreground">{report.satisfactionRating || 5}</span>
                    </div>
                  </div>
                </div>
              </div>
